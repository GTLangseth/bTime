...
class operating_time(object):
    ...
    def __init__(self,bt_start=9,bt_close=17,exclusion_list=[]):
        ...
        self.bday_us = cbd(calendar=self.cal)
    ...
    def list_us_bd(self,x, y):
        return pd.DatetimeIndex(start=x.date(), end=y.date(), freq=self.bday_us)

    def us_bd(self, x, y):
        try:
            val = len(pd.DatetimeIndex(start=x.date(), end=y.date(), freq=self.bday_us))
            return val
        except:
            return None
    ...
    def hr_in(self,x):
        hrIn = 0
        if self.is_b(x) and x.hour >= self.bt_start:
    ...
    def hr_out(self,x):
        hrOut = 0
        if self.is_b(x) and x.hour < self.bt_close:
